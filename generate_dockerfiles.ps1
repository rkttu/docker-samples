...
$TagList = ((Invoke-WebRequest -Uri "https://mcr.microsoft.com/v2/windows/servercore/tags/list" -UseBasicParsing).Content | ConvertFrom-Json).tags
...
foreach ($PS_VERSION in $PS_VERSION_LIST) {
    foreach ($OS_SKU in $OS_SKU_LIST) {
        $VER_LIST = $ServerCoreTagList | Where-Object { ($_ -clike "${OS_SKU}-*" -or $_ -eq "${OS_SKU}") -and $_ -cnotlike "*-amd64" }
        foreach ($VER in $VER_LIST) {
            $PowerShellCoreDockerfileTemplate = @"
# escape=``
...
"@;
            ...
            mkdir -Path $TARGET_PATH -Force
            [System.IO.File]::WriteAllLines("${TARGET_PATH}\Dockerfile", $PowerShellCoreDockerfileTemplate, $Utf8NoBomEncoding)
        }
    }
}
